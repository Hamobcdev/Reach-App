...
// Import algosdk with named exports for v2.7.x compatibility
...
async function compileTEALScript(): Promise<{ approvalPath: string; clearPath: string }> {
  try {
    console.log('\n📝 Compiling TEALScript to TEAL...');
    
    // Check if the contract file exists
    const contractPath = path.join(rootPath, 'contracts', 'CrisisManagement.algo.ts');
    ...
    try {
      ...
      const approvalPath = path.join(rootPath, 'contracts', 'approval.teal');
      const clearPath = path.join(rootPath, 'contracts', 'clear.teal');
...
// Read TEAL files
async function readTealFiles(approvalPath: string, clearPath: string): Promise<{ approvalProgram: string; clearProgram: string }> {
  try {
    console.log('\n📄 Reading TEAL files...');
...
// Deploy the contract
async function deployContract(): Promise<{ appId: number; appAddress: string; txId: string }> {
  try {
    ...
    // For algosdk v2.7.x, use makeApplicationCreateTxn
...
// Update .env file with app ID and address
function updateEnvFile(appId: number, appAddress: string, txId: string): void {
  try {
...
async function main() {
  try {
    // Log deployer account for debugging
